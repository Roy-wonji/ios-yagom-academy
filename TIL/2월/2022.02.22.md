...
코드를 구현을 하면서 아 이게 맞나 생각을 하면서 구현을 하게 됐는데 아는것을 생각 하면서 구현을 해야겠다라고 생각이 들었다 
저작권과 CCL 표기 그리고 오픈소스의 라이센스에 대해서 송부를 하고 상대방이 열심히 쓴글에 대해 내가 맘대로 갖다쓰면 저작권이 걸린다는걸 이해를 했다
